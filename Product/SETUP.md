...
The `postinstall` script will check that Stockfish files exist in `node_modules/stockfish/`. The app expects the pinned package's `stockfish-nnue-16-single.js` and matching WASM file to be copied into `dist/stockfish/`.

## 4 · Verify the AGPL-3.0 license text

The bundled `LICENSE` contains the canonical GNU AGPL-3.0 text. Before public release, confirm that the file is still present and unmodified.
...
- **Stockfish files missing from `dist/stockfish/`** → check `vite.config.ts` static-copy targets and your `node_modules/stockfish/` layout. The bundled local engine is required.
...
**Stockfish 404 on `/stockfish/stockfish-nnue-16-single.js`.** The local copy step failed. Inspect `node_modules/stockfish/src/` and adjust `vite.config.ts` static-copy targets so the engine JS and WASM land in `dist/stockfish/`.
