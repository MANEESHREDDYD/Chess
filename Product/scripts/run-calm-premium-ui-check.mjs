/**
 * M-PREMIUM-CALM-INTERFACE-RESTART-2 — Calm Premium UI screenshot + QA check.
 *
 * Captures every primary route across the required viewport matrix in BOTH the
 * Graphite Night (dark) and Ivory Studio (light) appearance themes, plus the
 * open More menu, open Board Theme menu and the bottom appearance toggle.
 * Runs the Phase-13 automated checks (no native header select, menus open,
 * Board Theme changes state, theme root switches, no horizontal overflow,
 * header does not cover content).
 *
 * Artifacts: Product/artifacts/calm-premium-ui/
 */
...
const ARTIFACT_DIR = path.resolve('artifacts/calm-premium-ui');
...
async function captureRoute(page, route, viewport, theme, failures) {
  ...
  const probe = await page.evaluate(() => {
    ...
    // Raw default links/buttons: an <a>/<button> with the browser default blue
    // and no app class is a prototype smell.
    return {
