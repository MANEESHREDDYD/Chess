...
const ARTIFACT_DIR = path.resolve('artifacts/premium-ux');
...
async function run() {
  ...
  try {
    ...
    for (const viewport of VIEWPORTS) {
      ...
      for (const route of ROUTES) {
        try {
          ...
          await verifyRoute(page, route, viewport, fileName);
    ...
    if (failures.length > 0) {
      throw new Error(`Premium UX design check failed:\n${failures.map((entry) => `- ${entry}`).join('\n')}`);
    }

    console.log('Premium UX design check passed.');
    console.log(`Screenshots saved to ${ARTIFACT_DIR}`);
  } catch (error) {
    console.error('Premium UX design check failed:', error);
    process.exitCode = 1;
  } finally {
...
async function verifyRoute(page, route, viewport, fileName) {
  ...
  const common = await page.evaluate(() => {
    ...
    const theme = rect('.theme-toggle select');
  ...
  if (!common.theme || common.theme.width < 90 || common.theme.height < 30) throw new Error('Theme control is missing or visually broken.');
