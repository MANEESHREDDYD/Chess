    ...
    for player in player_rows:
        ...
        lines.extend(
            [
                ...
                f"- Most used clue level: {player.get('most_used_clue_level') or 'insufficient data'}",
                f"- Solved without reveal rate: {as_percent(player.get('solved_without_reveal_rate', 0))}",
                f"- Review-mode success rate: {as_percent(player.get('review_mode_success_rate', 0))}",
                f"- Best clue streak: {player.get('best_clue_streak', 0)}",
                f"- Boss completions: {player.get('boss_completion_count', 0)}",
