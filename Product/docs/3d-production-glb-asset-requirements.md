...
Status: First Blender-generated GLB pack present. Final realism is still blocked until approved rigged/PBR model files and authored animations exist.

## Why This Is Required

The battlefield now uses true 3D mesh units and can load the complete Blender-generated GLB pack in `kurukshetra-production-v1`. The user's references require realistic/stylized-realistic warriors, horses, elephants, chariots, armor, cloth, weapons, and combat motion. The generated pack is an implementation step, not the final visual target.
...
The required 12 GLB files are present and generated by:

`Product/scripts/generate-kurukshetra-production-glbs.py`

They are original project-authored Blender geometry and are now the runtime 3D model pack. They are still not final AAA/film/game-quality assets. The next realism milestone should replace or upgrade them with approved PBR materials, higher-detail character/animal sculpts, rigs, and authored combat animation clips.
