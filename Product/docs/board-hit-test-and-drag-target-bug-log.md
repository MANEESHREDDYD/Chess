...
## 2026-06-12 (second report) — "3 blocks away": backdrop-filter containing block

**Report:** dragged coin still rendered ~3 squares from the cursor (dark + Classic +
black orientation, then Kurukshetra; ring correct under the pointer).

**Forensics (mid-drag DOM dump, `scratch/probe-drag-layer.mjs`):** the drag layer is a
CLASSLESS `position: fixed` div (no `data-piece` — invisible to attribute-based checks),
and the ancestor audit found `.play-board-card` carrying
`backdrop-filter: blur(16px) saturate(1.08)` from the glass-card polish.
**`backdrop-filter` creates a containing block for `position: fixed`** exactly like
`transform`/`filter` do, so the preview anchored to the card instead of the viewport.
The route-wrapper transform fixed earlier the same day was the FIRST creator; this glass
card was a second, independent one.

**Fix:** hard invariant in `mirrorAppleMono.css` — every board-hosting container
(`.play-board-card`, board wrap/shell/stage/frame, Clue/Review/Story board areas, and any
`.ui-card:has(.board-stage)`) forces `backdrop-filter / filter / transform / perspective /
will-change / contain` off with `!important`. This is a correctness invariant: board
ancestors must never become containing blocks, regardless of future styling passes.

**Guards added to the loop (every drag scenario):**
- ancestor audit — fails if ANY board ancestor computes a containing-block-creating
  property;
- preview detection now also matches the classless fixed drag-layer node (small fixed
  element containing an img/svg) and asserts ≤90px from the cursor (origin-parked layers
  from headless CDP drags, which receive no drag offset, are exempted — the ancestor
  audit is the deterministic guard);
- new scenario at the reported conditions: dark + Classic + BLACK orientation + scrolled
  (g7→g5), screenshot `play-dark-scrolled-black-during-drag.png`.

**Verified:** post-fix forensic dump shows zero containing-block ancestors; full
reference-locked loop passes.
